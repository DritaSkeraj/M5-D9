...
router.post("/:id/upload", upload.single("product"), async (req, res, next) => {
  try {
    ...
    addImgpProperty(
      req.params.id,
      `http://localhost:3001//img/products/` + `${req.params.id}.jpg`
    );
...
router.post(
  "/uploadMultiple",
  upload.array("multiple", 8),
  async (req, res, next) => {
    try {
      const arrayOfPromises = req.file.map((file) =>
        writeFile(
          path.join(productsFileImgPath, req.file.originalname),
          req.file.buffer
        )
      );
...
router.post(
  "/",
  [
    check("name").exists().withMessage("Add a name for the product please!"),
    check("description").exists().withMessage("Provide a description please!"),
    check("brand").exists().withMessage("Add the brand please!"),
    check("price").exists().withMessage("Provide the price"),
  ],
  async (req, res, next) => {
    try {
